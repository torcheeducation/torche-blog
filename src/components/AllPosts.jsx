...
export default function AllPosts() {
  ...
  return (
    <div className="my-10">
      <h2 className="text-2xl text-slate-500 uppercase">Semua Postingan</h2>
      <div className="mt-10 grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {posts.slice(0, numPost).map(({ id, date, title, text, image }) => (
          <Link key={id} href={`/posts/${id}`} className="group grid grid-cols-1 gap-4">
            <Image 
              src={image}
              alt={title}
              width={250}
            ...
            <div>
              <div className="flex flex-col justify-center">
                ...
                <h2 className="font-bold capitalize line-clamp-3 text-lg group-hover:text-blue-600">{title}</h2>
              </div>
              <div className="mt-2">
      ...
      {posts.slice(0, numPost).length !== posts.length && (
        <div className="mt-20 text-center">
          <button className="py-3 px-6 bg-searchIcon text-white font-bold rounded-3xl shadow-lg shadow-indigo-500/40" onClick={() => setNumPost((prevNumPost) => prevNumPost + 3)}>Muat Lebih Banyak</button>
        </div>
      )}
    </div>
  )
