...
export default function Education({ posts }) {
  const dataPost = posts.filter((p) => p.category === "edukasi")
                ...
                <Link href={`/posts/${_id}`} className="bg-slate-50">
                  ...
                  <div className="bg-slate-100 p-1 rounded-b-lg">
                    <div className="justify-center">
                      <p className="pt-4 pl-3 text-sm text-blueDate capitalize">{date}</p>
        ...
        <div className="grid col-span-4 gap-7 md:col-span-7 md:gap-7 lg:col-span-8 lg:gap-11 lg:grid-rows-3">
          {dataPost.slice(1, 4).map(
            ({ _id, title, description }) => {
              return (
                <article key={_id} className="rounded-t-lg border-t-[16px] border-[#9284F1] md:rounded-l-lg md:border-t-[0px] md:border-l-[50px] md:max-h-32 lg:max-h-36">
