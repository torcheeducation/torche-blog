...
export default function News() {
  const [numNewsLeft] = useState(1);
  const [numNewss] = useState(3);
    ...
    <div className=" mt-11 px-4 md:px-14">
      ...
      <div className=" mt-8 grid md:grid-cols-12  md:gap-7 lg:gap-11">
                ...
                <Link key={id} href={`/posts/${id}`} className="  bg-slate-50">
                  ...
                  <div className="bg-slate-100 p-1">
                    <div className="justify-center">
                      <p className="pt-4 pl-3 text-sm text-[#FFAE5F]">{date}</p>
        ...
        <div className="grid gap-7 md:col-span-7 md:gap-7 lg:col-span-8 lg:gap-11">
          {News.slice(0, numNewss).map(({ id, title, description }) => {
            return (
              <article key={id} className="rounded-t-lg border-t-[16px] border-[#FFAE5F] md:rounded-l-lg md:border-t-[0px] md:border-l-[50px]">
