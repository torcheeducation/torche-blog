...
export default function Education() {
  const [numEducationLeft] = useState(1);
  const [numEducation] = useState(3);
  ...
  return (
    <div className=" mt-5 px-4 md:px-14">
      ...
      <div className=" mt-8 grid md:grid-cols-12  md:gap-7 lg:gap-11">
        {Educations.slice(0, numEducationLeft).map(
                ...
                <Link key={id} href={`/posts/${id}`} className="  bg-slate-50">
                  ...
                  <div className="bg-slate-100 p-1">
                    <div className="justify-center">
                      <p className="pt-4 pl-3 text-sm text-blueDate">{date}</p>
        ...
        <div className="grid gap-7 md:col-span-7 md:gap-7 lg:col-span-8 lg:gap-11">
          {Educations.slice(0, numEducation).map(
            ({ id, title, description }) => {
              return (
                <article key={id} className="rounded-t-lg border-t-[16px] border-[#9284F1] md:rounded-l-lg md:border-t-[0px] md:border-l-[50px]">
